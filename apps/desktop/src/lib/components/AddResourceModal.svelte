			...
			<div class="mt-6 grid gap-4">
				...
				{#if resourceType === 'git'}
					<div>
						<label
							for="git-url"
							class="mb-2 block text-xs font-semibold tracking-[0.2em] uppercase"
						>
							Git URL
						</label>
						<input
					...
					{#if resourceType === 'git'}
						<div>
							...
							<div class="relative">
								<GitBranch
									size={14}
									class="pointer-events-none absolute top-1/2 left-3 -translate-y-1/2 text-[hsl(var(--bc-fg-muted))]"
								/>
								<input
									id="resource-branch"
									type="text"
